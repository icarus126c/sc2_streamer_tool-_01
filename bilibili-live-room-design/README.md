...
如果线上 API 更新不及时，或者国服 replay 里的自身 MMR 显示成固定 `2800` / 异常负数，可以在控制台打开 `Replay MMR估算`。它不是暴雪官方精确公式，也不会自动用 replay 里的自身 MMR 校准；默认参数来自 SC2 Pulse 公开逐局 `ratingChange` 样本拟合：K 值约 `44`，分差尺度约 `850`。它会用 replay 里的对手 MMR、胜负和你当前显示的 `当前MMR` 做连续估算；如果实际结算有偏差，可以在控制台的 `MMR变化` 里手动填 `+18` / `-12` 来修正当前值。
